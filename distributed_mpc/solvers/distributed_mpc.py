...
def solve_rhc_distributed(
  ...
  theta_min,tau_max,tau_min,phi_max,phi_min,n_humans,n_dims=None,j_trial=None,equal_dist = False):
    ...
    while not np.all(distance_to_goal(x0, xf, n_agents, n_states) <= 0.1) and (loop < M):
        ...
        # rel_dists = util.compute_pairwise_distance(x0, x_dims, n_d=3) this function is depcrecated
        rel_dists = compute_pairwise_distance_nd_Sym(x0,x_dims,n_dims)
        ...
        graph = define_inter_graph_threshold(x0, radius, x_dims, ids, n_dims)
        ...
        for (
            ...
            for k in range(N + 1):
                ...
                if len(x_dims_local) != 1:
                    if equal_dist == True:
                        if (set(ids_) >= {100, 101}) and len(ids_) == 2:
    ...
    logging.info(
            ...
            f'{objective_val},{N},{dt},{radius},{centralized},{t_solve},{np.mean(t_solve_step)},{distance_to_goal(x0, xf, n_agents, n_states)},'
        )
    #Note: aggregating objective values from all sub-problems is incorrect. Do not use it.
